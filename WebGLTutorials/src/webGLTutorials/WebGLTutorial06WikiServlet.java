...
public class WebGLTutorial06WikiServlet  extends HttpServlet {
    private static final String primaryOutputJSP = "/webGLTutorial06Wiki.jsp";
    private static transient Logger logger = Logger.getLogger(WebGLTutorial06WikiServlet.class);
    ...
    throws ServletException, IOException {
        ...
        //logger.info("\ncme@ webGLTutorial06Servlet.java doPost");
        ...
        boolean tet2Icosa=false;
        key= "4by20";
        if(paramMap.containsKey(key)){
        	//logger.info("see the request for tetrahedron to Icosahedron only");
            numHandeled++;
            tet2Icosa=true;
        } 
        //else {
        //  	logger.info("no request to limit span");  	
        //}
        key= "canvasData";
        if(paramMap.containsKey(key)){
			try{
				...
				String[] paramValues = req.getParameterValues(key);
				String value=paramValues[0].substring(pre.length());
	        	logger.debug(paramValues[0]);
	        	logger.debug(value);
		...
		if(false==ajaxLogging){
	         ...
	         */
	        result_html.append(wgls.GetWebGLTutorial06(cullFace,tet2Icosa)); /* would call a function in the serrice, but am not adding anything myself */
